...
def plotMultipleLinesOneFigure(yLsts, legend_labels, xlabel, ylabel, title_name):
    '''
    plot two suplots   2X1 structure
    '''
    
    plt.figure()
    for i in range(0, len(yLsts)):
        xList = range(0, len(yLsts[i]))
        print("xlst yLsts[i]: ", xList, yLsts[i], legend_labels[i])
        plt.plot(xList, yLsts[i], label=legend_labels[i])  #  legend_labels[i])
