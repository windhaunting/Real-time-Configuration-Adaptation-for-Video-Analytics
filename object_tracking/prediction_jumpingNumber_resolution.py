...
n_components = 30   # PCA componenet
...
class ModelClassifier(object):
    ...
    def test_on_multiple_mixed_video(self, video_dir_lst, min_acc):
       ...
       #predicted_video_dir = 'output_021_dance/'     # select different video id for testing
        ...
        for i, video_dir in enumerate(video_dir_lst[0:23]):    # combine all to generate more data
            ...
            data_pickle_dir = video_dir + "/jumping_number_result/" 
    ...
    def train_rest_test_one_video_one_model(self, video_dir_lst, predicted_video_dir, min_acc_threshold, single_featue):
        ...
        for i, video_dir in enumerate(video_dir_lst[0:38]):             # combine all to generate more data
            ...
            if single_featue == 'objectSizeChange':
                data_sub_dir = video_dir + "/jumping_number_result_" + single_featue + "/" + "jumpingNumber_resolution_selection_" + single_featue + "/"
            else:
                data_sub_dir = video_dir + "/jumping_number_result/jumpingNumber_resolution_selection/"
    ...
    def unbalanced_processing(self, X, y):
        ...
        oversample = SMOTE(n_jobs=-1, k_neighbors=5)  # , kind='regular', k_neighbors=2)   # 
        X1 = np.copy(X)
    ...
    def get_rf_model_train_two_models(self, X1, y1, X2, y2):
        ...
        # X, y are the trianing and validation data
        # X, y are the trianing and validation data
        pca1 = PCA(n_components=min(n_components, X1.shape[1])).fit(X1)
        ...
        pca2 = PCA(n_components=min(n_components, X2.shape[1])).fit(X2)
    ...
    def train_rest_test_one_video_two_models(self, video_dir_lst, predicted_video_dir, min_acc_threshold, single_featue):
        ...
        y_test = None
        first_flag = True  # first video
        for i, video_dir in enumerate(video_dir_lst[0:30]):             # combine all to generate more data
            ...
            if predicted_video_dir[:-1] == video_dir:        # get this video for test
                X_test = X
                y_test = y
        ...
        unbalanced_flag = True  # unbalanced processing

        if unbalanced_flag:
            x_input_arr1, y_arr1, x_input_arr2, y_arr2 = self.unbalanced_processing(x_input_arr, y_arr)
        else:
            x_input_arr1 = x_input_arr
            x_input_arr2 = x_input_arr
            y_arr1 = y_arr[:, 0]
            y_arr2 = y_arr[:, 1]
        ...
        if not os.path.exists(write_subDir):
            os.mkdir(write_subDir)
        print("write_subDir:" , write_subDir, predicted_video_dir)
        

        model_file = write_subDir + "model_classifier_frame_rate_joblib_exclusive_" + str(predicted_video_dir.split('/')[-2])  + ".pkl"
        
        #x_input_arr_file = write_subDir + "all_other_trained_x_instances.pkl"
        #write_pickle_data(x_input_arr, x_input_arr_file)
        _ = joblib.dump(mor1, model_file, compress=4)
        
        model_file = write_subDir + "model_classifier_resolution_joblib_exclusive_" + str(predicted_video_dir.split('/')[-2])  + ".pkl"
