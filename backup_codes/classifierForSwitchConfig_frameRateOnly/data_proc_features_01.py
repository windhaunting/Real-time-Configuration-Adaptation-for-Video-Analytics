...
#  the output is only resolution  few output only consider one model cmu
...
frameRate_dict = {r: i for i, r in enumerate(frameRates)}
print ("frameRate_dict : ", frameRate_dict)
...
def getPersonEstimation(est_res, personNo):
    ...
    input: 
        ...
        [6, 172, 2, 16, 164, 2, 6, 162, 2, 48, 182, 2, 0, 0, 0, 68, 266kk`56, 2, 10, 254, 2, 112, 312, 2, 0, 0, 0, 168, 328, 2, 0, 0, 0, 70, 412, 2, 28, 420, 2, 108, 600, 2, 0, 0, 0, 144, 692, 2, 0, 0, 0],1.283275842666626;
...
def getOnePersonFeatureInputOutput01(data_pose_keypoint_dir, data_pickle_dir,  history_frame_num, max_frame_example_used, minAccuracy, minDelayTreshold):
    ...
    #print ('cmu_model_index: ', len(cmu_model_index_lst), spf_frame_arr.shape)
    ...
    y_config_out_arr = np.zeros((max_frame_example_used+1), dtype=int)
    
    y_frmRate_out_arr = np.zeros((max_frame_example_used+1), dtype=int)        # only output resolution
    ...
    for index in range(0, len(df_det)):  
        ...
        if previous_frm_indx > history_frame_num:
            ...
            y_config_out_arr[select_frm_cnt+1], current_delay = select_config(acc_frame_arr, spf_frame_arr, history_frame_num, select_frm_cnt+1, switching_config_inter_skip_cnts+skipped_frm_cnt, minAccuracy, current_delay, minDelayTreshold)
                        
            current_cofig_id = int(y_config_out_arr[select_frm_cnt])
            
            current_cofig = id_config_dict[current_cofig_id]
            
            #print ("current_cofig: ", y_config_out_arr[select_frm_cnt+1], id_config_dict[y_config_out_arr[select_frm_cnt+1]])
            next_frmRate = int(id_config_dict[y_config_out_arr[select_frm_cnt+1]].split('-')[1])

            next_frmRt_id = frameRate_dict[next_frmRate]
            
            y_frmRate_out_arr[select_frm_cnt+1] = next_frmRt_id
    ...
    y_frmRate_out_arr = y_frmRate_out_arr[history_frame_num+1:select_frm_cnt+1]

    print ("input_x_arr xxxx, ", select_frm_cnt, input_x_arr, input_x_arr.shape, feature1_speed_arr.shape, feature2_relative_speed_arr.shape, current_delay)

    print ("y_frmRate_out_arr: ", y_frmRate_out_arr)
    
    return input_x_arr, y_frmRate_out_arr
...
def getOnePersonFeatureInputOutput02(data_pose_keypoint_dir, data_pickle_dir,  history_frame_num, max_frame_example_used, minAccuracy, minDelayTreshold):
    ...
    for index in range(0, len(df_det)):  
        ...
        if previous_frm_indx> history_frame_num:
            ...
            y_config_out_arr[select_frm_cnt+1], current_delay = select_config(acc_frame_arr, spf_frame_arr, history_frame_num, select_frm_cnt+1, switching_config_inter_skip_cnts+skipped_frm_cnt, minAccuracy, current_delay, minDelayTreshold)
            current_cofig_id = int(y_config_out_arr[select_frm_cnt+1])
            
            current_cofig = id_config_dict[int(y_config_out_arr[select_frm_cnt])]
    ...
    y_config_out_arr = y_config_out_arr[history_frame_num+1:select_frm_cnt+1]
    ...
    return input_x_arr, y_config_out_arr
...
def getOnePersonFeatureInputOutput03(data_pose_keypoint_dir, data_pickle_dir,  history_frame_num, max_frame_example_used, minAccuracy, minDelayTreshold):
    ...
    for index in range(0, len(df_det)):  
        ...
        if previous_frm_indx > history_frame_num:
            ...
            #current_cofig = id_config_dict[int(y_config_out_arr[index])]
            
            #print ("current_cofig: ", current_cofig)
            #frmRt = int(current_cofig.split('-')[1])

            history_config_arr[select_frm_cnt] = y_config_out_arr[select_frm_cnt]
                
            curr_config_aver= getConfigFeature(history_config_arr, prev_config_aver)
            
            prev_config_aver = curr_config_aver
            
            config_feature_arr[select_frm_cnt] =  y_config_out_arr[select_frm_cnt]  # y_config_out_arr[select_frm_cnt]  #   #curr_config_aver
...
def getOnePersonFeatureInputOutput04(data_pose_keypoint_dir, data_pickle_dir,  history_frame_num, max_frame_example_used, minAccuracy, minDelayTreshold):
    ...
    for index in range(0, len(df_det)):  
        ...
        if previous_frm_indx > history_frame_num:
            ...
            reso_feature_arr[select_frm_cnt] =  curr_reso_aver# curr_reso_aver y_config_out_arr[index]    #curr_reso_aver
...
def select_config(acc_frame_arr, spf_frame_arr, history_frame_num, index_id, switching_frm_num,  minAccuracy, current_delay, minDelayTreshold):
    ...
    cmu_model_index_lst = [0, 2, 4, 6, 8, 10, 12, 15, 17, 19, 21, 24, 27, 29, 31, 33, 37, 39, 42, 44, 47, 49, 52, 54, 56, 58, 61,
     63,66, 69]
    ...
    for r in indx_acc_selected_arr[0][::-1] :    # descending
        ...
        if indx_config_above_minAcc[0][r] not in cmu_model_index_lst:
            continue
        ...
        if current_delay_cpy < minDelayTreshold:
            #print ("rrrrrrrr: ", index_id, switching_frm_num, indx_config_above_minAcc[0][r], current_delay_cpy,  spf_frame_arr[indx_config_above_minAcc[0][r]][index_id])
            selected_config_indx = indx_config_above_minAcc[0][r]
...
def getDataExamples():
    ...
    for video_dir in video_dir_lst[0:1]:    # [2:3]:   #[1:2]:  # [1:2]:  #[0:1]:        #[1:2]:
        ...
        x_input_arr, y_config_out_arr = getOnePersonFeatureInputOutput01(data_pose_keypoint_dir, data_pickle_dir,  history_frame_num, max_frame_example_used, minAccuracy, minDelayTreshold)
        
        #x_input_arr, y_config_out_arr = getOnePersonFeatureInputOutput02(data_pose_keypoint_dir, data_pickle_dir,  history_frame_num, max_frame_example_used, minAccuracy, minDelayTreshold)
        #x_input_arr, y_config_out_arr = getOnePersonFeatureInputOutput03(data_pose_keypoint_dir, data_pickle_dir,  history_frame_num, max_frame_example_used, minAccuracy, minDelayTreshold)
        
        #x_input_arr, y_config_out_arr = getOnePersonFeatureInputOutput04(data_pose_keypoint_dir, data_pickle_dir,  history_frame_num, max_frame_example_used, minAccuracy, minDelayTreshold)
        #x_input_arr, y_config_out_arr = getOnePersonFeatureInputOutput05(data_pose_keypoint_dir, data_pickle_dir,  history_frame_num, max_frame_example_used, minAccuracy, minDelayTreshold)
        
        #y_config_out_arr = getGroundTruthY(data_pickle_dir, max_frame_example_used, history_frame_num)
        x_input_arr = x_input_arr.reshape((x_input_arr.shape[0], -1))
        
        # add current config as a feature
        print ("combined before:",x_input_arr.shape, y_config_out_arr[history_frame_num:-1].shape)
        #current_config_arr = y_config_out_arr[history_frame_num:-1].reshape((y_config_out_arr[history_frame_num:-1].shape[0], -1))
        #x_input_arr = np.hstack((x_input_arr, current_config_arr))
        
        #y_config_out_arr = y_config_out_arr[history_frame_num+1:]
        
        print ("y_config_out_arr shape after:", x_input_arr.shape, y_config_out_arr.shape)
        
        #data_examples_arr = np.hstack((x_input_arr, y_config_out_arr))
        ...
        with open(out_frm_examles_pickle_dir + "Y_data_features_config-history-frms" + str(history_frame_num) + "-sampleNum" + str(max_frame_example_used) + ".pkl", 'wb') as fs:
            pickle.dump(y_config_out_arr, fs)
