...
def getOnePersonFeatureInputOutput01(data_pose_keypoint_dir, data_pickle_dir,  history_frame_num, max_frame_example_used, minAccuracy, minDelayTreshold):
    ...
    y_reso_out_arr = np.zeros((max_frame_example_used+1), dtype=int)        # only output resolution
    ...
    for index in range(0, len(df_det)):  
        ...
        if previous_frm_indx > history_frame_num:
            ...
            next_reso = int(id_config_dict[y_config_out_arr[select_frm_cnt+1]].split('-')[0].split('x')[1])

            next_reso_id = resolution_dict[next_reso]
            
            y_reso_out_arr[select_frm_cnt+1] = next_reso_id
    ...
    y_reso_out_arr = y_reso_out_arr[history_frame_num+1:select_frm_cnt+1]
    ...
    print ("y_reso_out_arr: ", y_reso_out_arr)
    
    return input_x_arr, y_reso_out_arr
