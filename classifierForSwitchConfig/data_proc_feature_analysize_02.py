...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 19 15:02:35 2019
...
def getCurrentAverageSpeed(history_pose_est_arr, used_current_frm, prev_history_difference, all_kp_flag):
    # only get 8 key points most important points
    
    used_prev_frm = used_current_frm - prev_history_difference #  (PLAYOUT_RATE)   # 1sec
    
    j = used_current_frm
    
    if not all_kp_flag:
        
        indx_key_points_fetched = [7, 8, 9, 10, 13, 14, 15, 16]
    
        dim_num = len(indx_key_points_fetched)  # history_pose_est_arr.shape[1]
        current_speed_angle_arr = np.zeros(dim_num)
        while (j > used_prev_frm):
            cur_frm_est_arr = history_pose_est_arr[j][indx_key_points_fetched]    
            prev_frm_est_arr = history_pose_est_arr[j-1][indx_key_points_fetched] 
            
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 
            hstack_arr = np.hstack((cur_frm_est_arr, prev_frm_est_arr))
            
            #print ("current_speed_angle_arr: ", current_speed_angle_arr.shape)
            time_frm_interval = (1.0/PLAYOUT_RATE)      # because here it's one frame interval *difference       # 1 sec  #(1.0/PLAYOUT_RATE)/2            # interval 1 sec
    ...
    current_speed_angle_arr = current_speed_angle_arr
    
    return current_speed_angle_arr


def getCurrentAverageRelativeSpeed(history_pose_est_arr, used_current_frm, prev_history_difference):
    
    used_prev_frm = used_current_frm-prev_history_difference       
    ...
    while (j > used_prev_frm):
        cur_frm_est_arr = history_pose_est_arr[j]    
        prev_frm_est_arr = history_pose_est_arr[j-1]
        time_frm_interval =  (1.0/PLAYOUT_RATE)  #    *prev_history_difference  # (1.0/PLAYOUT_RATE)          # interval difference frame time
        current_speed_angle_arr += relativeSpeed(cur_frm_est_arr, prev_frm_est_arr, time_frm_interval)
        j -= 1
    
    current_speed_angle_arr = current_speed_angle_arr  # /prev_history_difference
    return current_speed_angle_arr

    
def getFeatureOnePersonMovingSpeed(history_pose_est_arr, used_current_frm, prev_EMA_speed_arr, prev_history_difference, all_kp_flag):
    '''
    ...
    euclidean d distance of current frame with the previous frame {f j−m , m =
                                                                   1, 2, ..., 24
    only get the previous segment_begin_interval, that is current_frm_id is the current frame id
    ...
    current_speed_angle_arr = np.apply_along_axis(getEuclideanDist, 1, hstack_arr, time_frm_interval)    
    '''
    current_speed_angle_arr = getCurrentAverageSpeed(history_pose_est_arr, used_current_frm, prev_history_difference, all_kp_flag)
...
def getFeatureOnePersonRelativeSpeed1(history_pose_est_arr, used_current_frm, prev_EMA_relative_speed_arr, prev_history_difference):
    '''
    ...
    output: feature2_relative_speed_arr 5 x segment_begin_interval-1
    ...
    current_speed_arr = getCurrentAverageRelativeSpeed(selected_kp_history_pose_est_arr, used_current_frm, prev_history_difference)
...
def getFeatureOnePersonRelativeSpeed2(history_pose_est_arr, used_current_frm, prev_EMA_relative_speed_arr, prev_history_difference):
    '''
    feature 2 One person arm/feet’s relative speed to the torso of all keypoints
    with the previous frames.
    In each frame, we select the relative distance of left wrist, right wrist,left ankle
...
def getFeatureOnePersonRelativeSpeed3(history_pose_est_arr, used_current_frm, prev_EMA_relative_speed_arr, prev_history_difference):
    '''
    feature 2 One person arm/feet’s relative speed to the torso of all keypoints
    with the previous frames.
    In each frame, we select the relative distance of left wrist, right wrist,left ankle
...
def getFeatureOnePersonRelativeSpeed4(history_pose_est_arr, used_current_frm, prev_EMA_relative_speed_arr, prev_history_difference):
    '''
    feature 2 One person arm/feet’s relative speed to the torso of all keypoints
    with the previous frames.
    In each frame, we select the relative distance of left wrist, right wrist,left ankle
...
def getOnePersonFeatureInputOutputAll001(data_pose_keypoint_dir, data_pickle_dir, data_frame_path_dir, segment_begin_interval, prev_history_difference, max_frame_example_used, minAccuracy, minDelayTreshold):
    '''
    ...
    start from segment_begin_interval;
    ...
    max_frame_example_used = min(max_frame_example_used, confg_est_frm_arr.shape[1])      # determine the number of features
    ...
    if not all_kp_flag:
        input_x_arr = np.zeros((max_frame_example_used, 8)) #  33))   # , 2))    # np.zeros((max_frame_example_used, 66, 2))       # 17 + 4*4 + 4*4 + 17
        prev_EMA_speed_arr = np.zeros(8)       # only fetch four points wrist, elbow, knee, ankle, COCO_KP_NUM)
    
    else:
        input_x_arr = np.zeros((max_frame_example_used, 17)) #  33))   # , 2))    # np.zeros((max_frame_example_used, 66, 2))       # 17 + 4*4 + 4*4 + 17
        prev_EMA_speed_arr = np.zeros(17)       # only fetch four points wrist, elbow, knee, ankle, COCO_KP_NUM)
    ...
    used_current_frm = PLAYOUT_RATE
    used_prev_frm = used_current_frm          # because some frames have 0 persons detected because of dark, no person existing
    step_indx_first = 0
    while used_current_frm < num_frames:           # index_frm is 1 sec interval      
        ...
        if step_indx_first >= segment_begin_interval:#          and index_frm >= segment_begin_interval:              # jump interval;  jump the first interval/frame, because calculating speed from second to previous interval/frame
                        
            current_iterative_frm_id = used_current_frm #  + interval_jump               # the next interval for predicting is the class output
                        
            if minAccuracy == -1:
                delay_feature_arr[select_frm_cnt] = current_delay
                return_index, current_delay = select_config_boundedDelayOnly(acc_frame_arr, spf_frame_arr, current_iterative_frm_id, interval_frms, current_delay, minDelayTreshold)
                
            else:
                return_index = select_config_boundedAccuracy(acc_frame_arr, spf_frame_arr, current_iterative_frm_id, minAccuracy)
                
            
            if return_index !=-1:
                y_out_arr[select_frm_cnt] = return_index
                
                used_current_frm += interval_jump
                used_prev_frm = used_current_frm
            else:        # skip this instance for training/test data
                
                #used_prev_frm = used_current_frm
                used_current_frm += interval_jump
                continue
            used_prev_frm_speed_index = used_prev_frm-PLAYOUT_RATE-1
            ...
            feature1_speed_arr = getFeatureOnePersonMovingSpeed(history_pose_est_arr, used_prev_frm_speed_index, prev_EMA_speed_arr, prev_history_difference, all_kp_flag)
            prev_EMA_speed_arr = feature1_speed_arr

            '''
            feature1_relative_speed_arr = getFeatureOnePersonRelativeSpeed1(history_pose_est_arr, used_current_frm, prev_EMA_relative_speed_arr1, prev_history_difference)
            ...
            feature2_relative_speed_arr = getFeatureOnePersonRelativeSpeed2(history_pose_est_arr, used_current_frm, prev_EMA_relative_speed_arr2, prev_history_difference)
            prev_EMA_relative_speed_arr2 = feature2_relative_speed_arr
            
            
            feature3_relative_speed_arr = getFeatureOnePersonRelativeSpeed3(history_pose_est_arr, used_current_frm, prev_EMA_relative_speed_arr3, prev_history_difference)
            prev_EMA_relative_speed_arr3 = feature1_relative_speed_arr
            #print ("feature1_speed_arr feature2_relative_speed_arr, ", feature1_speed_arr.shape, feature2_relative_speed_arr.shape)
   
            feature4_relative_speed_arr = getFeatureOnePersonRelativeSpeed2(history_pose_est_arr, used_current_frm, prev_EMA_relative_speed_arr4, prev_history_difference)
            ...
            input_x_arr[select_frm_cnt]=  feature1_speed_arr  # feature1_speed_arr  # total_features_arr  #  input_x_arr[frm_id-1] = total_features_arr
...
#
            ...
            img_file_name = paddingZeroToInter(used_current_frm-PLAYOUT_RATE+1) + '.jpg'    # no start from index 0, 000001.jpg start. from the 2nd sec to have output
            ...
            #print ("current_instance_start_frm_path_arr[select_frm_cnt]   ", data_frame_path_dir + img_file_name, current_instance_start_frm_path_arr[select_frm_cnt] )
        ...
        if used_current_frm > (max_frame_example_used-1):
            break 
    ...
    #print ("current_instance_start_frm_path_arr, ", current_instance_start_frm_path_arr.shape, current_instance_start_frm_path_arr)
    ...
    return input_x_arr, y_out_arr, current_instance_start_frm_path_arr, id_config_dict, acc_frame_arr, spf_frame_arr, confg_est_frm_arr
...
def select_config_boundedAccuracy(acc_frame_arr, spf_frame_arr, index_id, minAccuracy):
    ...
    #print("indx_config_above_minAcc: ", indx_config_above_minAcc, len(indx_config_above_minAcc[0]))
            # in case no profiling config found satisfying the minAcc, but        # at least ground truth satisfied  
    # so if possilbe no human existed or detected
    if len(indx_config_above_minAcc[0]) == 0:        # select a config with maximum accuracy, because not considering bounded delay
        #print ("select_config_boundedAccuracy acc_frame_arr: ", index_id, acc_frame_arr[:, index_id], minAccuracy,  spf_frame_arr[:, index_id])
        #print ("select_config_boundedAccuracy selected: ", np.argmin(spf_frame_arr[:, index_id]))

        return -1  # np.argmin(spf_frame_arr[:, index_id]) #np.argmax(acc_frame_arr[:, index_id])   # selected the minimum spf, i.e. the fastest processing speed
    ...
    #if index_id==3100:
    #    print ("acc_frame_arracc_frame_arracc_frame_arr", index_id, acc_frame_arr[:, index_id], spf_frame_arr[:, index_id], selected_config_indx)
...
def execute_get_feature_config_boundedDelay(data_pose_keypoint_dir, data_pickle_dir, data_frame_path_dir, segment_begin_interval, prev_history_difference, max_frame_example_used, feature_calculation_flag):
    '''
    ...
    if feature_calculation_flag == 'most_expensive_config':
        ...
        from data_proc_feature_analysize_02 import getOnePersonFeatureInputOutputAll001
    ...
    x_input_arr, y_out_arr, instance_start_frm_path_arr, id_config_dict, acc_frame_arr, spf_frame_arr, confg_est_frm_arr = getOnePersonFeatureInputOutputAll001(data_pose_keypoint_dir, data_pickle_dir, data_frame_path_dir, segment_begin_interval, prev_history_difference, max_frame_example_used, -1, minDelayTreshold)
    ...
    return x_input_arr, y_out_arr, instance_start_frm_path_arr, id_config_dict
        
  
def execute_get_feature_config_boundedAcc(data_pose_keypoint_dir, data_pickle_dir, data_frame_path_dir, segment_begin_interval, prev_history_difference, max_frame_example_used, feature_calculation_flag, minAccuracy):
    '''
    most expensive config's pose result to get feature
    '''
    ...
    if feature_calculation_flag == 'most_expensive_config':
        from data_proc_feature_analysize_02 import getOnePersonFeatureInputOutputAll001
    #x_input_arr, y_out_arr = getOnePersonFeatureInputOutput01(data_pose_keypoint_dir, data_pickle_dir,  segment_begin_interval, max_frame_example_used, minAccuracy)
    #x_input_arr, y_out_arr = getOnePersonFeatureInputOutput02(data_pose_keypoint_dir, data_pickle_dir,  segment_begin_interval, max_frame_example_used, minAccuracy)
    #x_input_arr, y_out_arr = getOnePersonFeatureInputOutput03(data_pose_keypoint_dir, data_pickle_dir,  segment_begin_interval, max_frame_example_used, minAccuracy)
    #x_input_arr, y_out_arr = getOnePersonFeatureInputOutput04(data_pose_keypoint_dir, data_pickle_dir,  segment_begin_interval, max_frame_example_used, minAccuracy)
    #x_input_arr, y_out_arr, _ = getOnePersonFeatureInputOutputAll001(data_pose_keypoint_dir, data_pickle_dir,  segment_begin_interval, max_frame_example_used, minAccuracy)
    x_input_arr, y_out_arr, instance_start_frm_path_arr, id_config_dict, acc_frame_arr, spf_frame_arr, confg_est_frm_arr = getOnePersonFeatureInputOutputAll001(data_pose_keypoint_dir, data_pickle_dir, data_frame_path_dir, segment_begin_interval, prev_history_difference, max_frame_example_used, minAccuracy, -1)
 
    x_input_arr = x_input_arr.reshape((x_input_arr.shape[0], -1))
            
    # add current config as a feature
    #print ("combined before:",x_input_arr.shape, y_out_arr.shape)
    #current_config_arr = y_out_arr[segment_begin_interval:-1].reshape((y_out_arr[segment_begin_interval:-1].shape[0], -1))
    #x_input_arr = np.hstack((x_input_arr, current_config_arr))
            
    #y_out_arr = y_out_arr[segment_begin_interval+1:]
    
    print ("y_out_arr shape after:", x_input_arr.shape, y_out_arr.shape, instance_start_frm_path_arr.shape)
...
def get_all_video_x_y_manual_feature():
    '''
    combine mutlipel data example together to get all feature X,y
    '''
    ...
    segment_begin_interval = 1       #1          # 
    
    prev_history_difference = PLAYOUT_RATE # PLAYOUT_RATE               #5 # PLAYOUT_RATE   # previous 1 sec
    ...
    xfile = "X_data_features_config" + "-sampleNum" + str(max_frame_example_used) + "-seg_frm" + str(seg_frm) + "-prev_history_difference" + str(prev_history_difference) + "-minAcc" + str(minAccuracy) + ".pkl"
    yfile = "Y_data_features_config" + "-sampleNum" + str(max_frame_example_used) + "-seg_frm" + str(seg_frm) + "-prev_history_difference" + str(prev_history_difference) + "-minAcc" + str(minAccuracy) + ".pkl"
    x_video_frm_path_file = "X_video_frm_path" + "-sampleNum" + str(max_frame_example_used) + "-seg_frm" + str(seg_frm) + "-prev_history_difference" + str(prev_history_difference) + "-minAcc" + str(minAccuracy) + ".pkl"
    if os.path.exists(data_classification_dir+xfile) and os.path.exists(data_classification_dir+yfile) and os.path.exists(data_classification_dir+x_video_frm_path_file):
        
        total_X, total_y= load_data_all_features(data_classification_dir, xfile, yfile)
        total_x_video_frm_path_lst = np.load(data_classification_dir + x_video_frm_path_file, allow_pickle=True)
        ...
        for i, video_dir in enumerate(video_dir_lst):  # [2:3]:     # [2:3]:   #[1:2]:      #[0:1]:     #[ #[1:2]:  #[1:2]:         #[0:1]:
            ...
            x_input_arr, y_out_arr, instance_start_frm_path_arr, id_config_dict = execute_get_feature_config_boundedAcc(data_pose_keypoint_dir, data_pickle_dir, data_frame_path_dir, segment_begin_interval, prev_history_difference, max_frame_example_used, 'most_expensive_config', minAccuracy)
            
            #x_input_arr, y_out_arr, instance_start_frm_path_arr, id_config_dict = execute_get_feature_config_boundedDelay(data_pose_keypoint_dir, data_pickle_dir, data_frame_path_dir, segment_begin_interval, prev_history_difference, max_frame_example_used, 'most_expensive_config')
    
    
                        
            #with open(out_frm_examles_pickle_dir + "X_data_features_config-history-frms" + str(segment_begin_interval) + "-sampleNum" + str(max_frame_example_used) + "-minAcc" + str(minAccuracy) + ".pkl", 'wb') as fs:
            #    pickle.dump(x_input_arr, fs)
                    
            #with open(out_frm_examles_pickle_dir + "Y_data_features_config-history-frms" + str(segment_begin_interval) + "-sampleNum" + str(max_frame_example_used) + "-minAcc" + str(minAccuracy) + ".pkl", 'wb') as fs:
            #    pickle.dump(y_out_arr, fs)
            
            #xfile = "X_data_features_config-history-frms" + str(segment_begin_interval) + "-sampleNum" + str(max_frame_example_used) + ".pkl"
            #yfile = "Y_data_features_config-history-frms" + str(segment_begin_interval) + "-sampleNum" + str(max_frame_example_used) + ".pkl" #'Y_data_features_config-history-frms1-sampleNum20000.pkl'    #'Y_data_features_config-history-frms1-sampleNum8025.pkl'
            ...
            x_video_frm_path_lst.append(instance_start_frm_path_arr)
            
        total_X = np.vstack(X_lst)
        total_y = np.vstack(y_lst)
        total_x_video_frm_path_lst =  np.vstack(x_video_frm_path_lst)
        print("total_X: ", total_X.shape, total_y.shape, data_pickle_dir)
        #print("total_x_video_frm_path_lst: ", total_x_video_frm_path_lst)
        data_pose_keypoint_dir =  dataDir3 + video_dir
        ...
        with open(data_classification_dir + xfile, 'wb') as fs:
            pickle.dump(total_X, fs)
                
        with open(data_classification_dir + yfile, 'wb') as fs:
            pickle.dump(total_y, fs) 
    
        with open(data_classification_dir + x_video_frm_path_file, 'wb') as fs:
            pickle.dump(total_x_video_frm_path_lst, fs) 
        
    generate_y_out_put(data_classification_dir, total_x_video_frm_path_lst, total_X, total_y)


    return total_X, total_y, total_x_video_frm_path_lst


if __name__== "__main__": 
            
    
    get_all_video_x_y_manual_feature()
