...
def getFeatureOnePersonMovingSpeed(history_pose_est_arr, current_frm_id, history_frame_num, prev_EMA_speed_arr):
    ...
    feature1_speed_arr = np.zeros(COCO_KP_NUM)
    ...
    for i in range(current_frm_id-1, current_frm_id-history_frame_num, -1):
        ...
        prev_frm_est_arr = history_pose_est_arr[i]
        ...
        current_speed_arr[j,:] = belta**(current_frm_id-i)*np.apply_along_axis(getEuclideanDist, 1, hstack_arr, time_frm_interval)
        j+= 1
...
def getFeatureOnePersonRelativeSpeed(history_pose_est_arr, current_frm_id, history_frame_num, prev_EMA_relative_speed_arr):
    ...
    for i in range(current_frm_id-1, current_frm_id-history_frame_num, -1):
        ...
        current_speed_arr[j, :] = belta ** (current_frm_id-i) * relativeSpeed( previous_arr, curr_frm_rel_dist_arr, time_frm_interval)
...
def getDataExamples():
    ...
    history_frame_num = 10          # 10
    max_frame_example_used = 9000   # 8000
