...
# -*- coding: utf-8 -*-
"""
Created on Mon May 25 22:59:41 2020
...
# after jumping frame number is decided, use cascaded way to connect another classifier for selecting resolution
...
class ResoDataGenerate(object):
    ...
    def get_data_instances(self, config_est_frm_arr_all, acc_frame_arr_all, spf_frame_arr_all, max_jump_number = 25, min_acc_threshold = 0.95, speed_type = 'ema', all_kp_flag = 1, interval_frm = 1):
        # get estimated speed    
        ...
        list_estimated_speed_2_reoslution = blist()           # speed => reso store resolution ans_reso_indx
        ...
        if speed_type == 'ema':
            ...
            while(current_indx < FRM_NO):
                ...
                if all_kp_flag == 1:        # use all the 17 keypoints  # absolute
                    ...
                    data_one_instance_frmRt = np.append(feature_x, count_jumping_frames)
                    # list_estimated_speed_2_jump_number.append([feature_x, end_frame_index])
                    #print("data_one_instance: ", data_one_instance)
                    list_estimated_speed_2_jump_number.append(data_one_instance_frmRt)
                        
                    
                    data_one_instance_reso = np.append(feature_x, ans_reso_indx)
                    list_estimated_speed_2_reoslution.append(data_one_instance_reso)
                ...
                #if current_indx > 1000:  # debug only
                #    break
        ...
        arr_estimated_speed_2_reso = np.asarray(list_estimated_speed_2_reoslution)
        ...
        return arr_estimated_speed_2_jump_number, arr_estimated_speed_2_reso, detected_est_frm_arr, segment_acc_arr, delay_up_arr 
    ...
    def getDataExamples(self):
        ...
        for i, video_dir in enumerate(video_dir_lst): # [3:4]):    # [2:3]:   #[1:2]:  # [1:2]:  #[0:1]:        #[1:2]:
            ...
            config_est_frm_arr, acc_frame_arr, spf_frame_arr = self.get_data_numpy(data_pose_keypoint_dir, data_pickle_dir)
            
            
            speed_type = 'ema'
            interval_frm = 1
            all_kp_flag = 1
            max_jump_number = float('inf')  # 25
            ...
            arr_estimated_speed_2_jump_number, arr_resolution_indices, detected_est_frm_arr, segment_acc_arr, delay_up_arr = self.get_data_instances(config_est_frm_arr, acc_frame_arr, spf_frame_arr, max_jump_number, min_acc_threshold, speed_type, all_kp_flag, interval_frm)
            
            
            print("all_arr_estimated_speed_2_resolution: ", arr_estimated_speed_2_jump_number.shape, arr_resolution_indices.shape)

            if i == 0:
                all_arr_estimated_speed_2_resolution = arr_resolution_indices
            else:
                all_arr_estimated_speed_2_resolution = np.vstack((all_arr_estimated_speed_2_resolution, arr_resolution_indices))
            ...
            out_pickle_dir =  dataDir3 + video_dir + "/jumping_number_result/" + "/resolution_prediction/"
            if not os.path.exists(out_pickle_dir):
                os.mkdir(out_pickle_dir)
            
                
            subDir = out_pickle_dir + "intervalFrm-" + str(interval_frm) + "_speedType-" + str(speed_type) + "_minAcc-" + str(min_acc_threshold) + "/"
            
            if not os.path.exists(subDir):
                os.mkdir(subDir)
                
     
            out_data_pickle_file = subDir + "data_instance_speed_resolution_xy.pkl" 
            write_pickle_data(arr_resolution_indices, out_data_pickle_file)
        ...
        #output_dir = dataDir3 + "dynamic_resolution_selection_output/"
        #if not os.path.exists(output_dir):
        #    os.mkdir(output_dir)
        #self.write_all_instance_data(output_dir, all_arr_estimated_speed_2_resolution)
    ...
    def write_all_instance_data(self, output_dir, all_arr_estimated_speed_2_jump_number):
        ...
        out_data_pickle_file = output_dir + "all_data_instance_speed_resolution_xy.pkl" 
        write_pickle_data(all_arr_estimated_speed_2_jump_number, out_data_pickle_file)
        
if __name__== "__main__": 
    
    data_obj = ResoDataGenerate()
